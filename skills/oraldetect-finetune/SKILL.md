...
DATAS=$REPO/OralDetect/datas                      # generated jsons + vocabulary
LAUNCH=$REPO/OralDetect/launch_bash               # generated yamls
...
```yaml
paths:
  wedetect:   $REPO/OralDetect/WeDetect
  config:     $REPO/OralDetect/WeDetect/config/oraldetect_finetune.py
...
Report the table and the figure together, with an honest reading:

- **F1 did not improve** — say so. Likely: too few boxes per class, `lr` unmatched to the starting
  checkpoint, too few epochs, or a val split too small for the difference to mean anything.
- **F1 improved but is still low in absolute terms** — say both numbers and the dataset size. A
  rise from near-zero to near-zero is not evidence the model is usable on this data.
- **precision up, recall down** (or the reverse) — the model became conservative or trigger-happy;
  the threshold may be what needs changing rather than the weights.
...
**mAP far below the README table** — the README reports MACRO-5 on the 5-modality benchmark;
`evaluator: coco` is standard COCO mAP on different data. Not comparable.
...
**Never modify** `$REPO/OralDetect/WeDetect/config/oraldetect_finetune.py`.
