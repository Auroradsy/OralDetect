...
cp $DATA/dl/Detect-Training/*.json OralDetect/datas/
```
...
Edit `OralDetect/launch_bash/finetune_oraldetect.yaml` — point it at your data. Nothing else
changes.

```yaml
paths:
  wedetect:   OralDetect/WeDetect
  config:     OralDetect/WeDetect/config/oraldetect_finetune.py
...
```bash
python run_finetune.py --config OralDetect/launch_bash/finetune_oraldetect.yaml --dry-run
```
...
torchrun --nproc_per_node=4 run_finetune.py \
    --config OralDetect/launch_bash/finetune_oraldetect.yaml
```
...
```bash
python run_eval.py --config OralDetect/launch_bash/eval_oraldetect.yaml --dry-run
torchrun --nproc_per_node=4 run_eval.py --config OralDetect/launch_bash/eval_oraldetect.yaml
```
