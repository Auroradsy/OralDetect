"""Build a WeDetect init checkpoint with OralCLIP-v14-clean's dental-CLIP-trained ConvNeXt
vision backbone injected, then DROP the XLM-R text encoder so the init pairs cleanly with
the DentalBertLanguageBackbone (which loads its own DentalBERT weights from HF at build).

Same vision remap logic as build_oralclip_init.py (timm convnext -> Facebook/mmpretrain
convnext), only the OralCLIP checkpoint (v14-clean) and the extra text-drop step
differ. Vision copied only where shapes match; unmatched WeDetect tensors keep the base
init. After injection, every `backbone.text_model.model.*` (XLM-R encoder) key is deleted;
`backbone.text_model.head.*`, neck, and bbox_head are kept."""
import re, torch

WD = "/ihome/lzhan/sid51/projects/VLM/OralDetect-Family/pretrained_models/wedetect_base.pth"
OC = "/ix/lzhan/siyuan/exps/OralDetect_Family/our_ckpts/oralclip/clip_checkpoints_v14_clean/best.pt"
OUT = "/ihome/lzhan/sid51/projects/VLM/OralDetect-Family/pretrained_models/wedetect_oralclipv14clean_dentalbert_init.pth"
